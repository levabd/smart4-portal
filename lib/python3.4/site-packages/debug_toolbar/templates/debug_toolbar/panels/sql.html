...
<table>
    ...
    <tbody>
        ...
        <tr class="djDebugHoverable {% cycle 'djDebugOdd' 'djDebugEven' %}{% if query.is_slow %} djDebugRowWarning{% endif %}{% if query.starts_trans %} djDebugStartTransaction{% endif %}{% if query.ends_trans %} djDebugEndTransaction{% endif %}{% if query.in_trans %} djDebugInTransaction{% endif %}"
            ...
            <td class="timeline">
                <div class="djDebugTimeline">
                    <div class="djDebugLineChart{% if query.is_slow %} djDebugLineChartWarning{% endif %}" data-left="{{ query.start_offset|unlocalize }}%"><strong data-width="{{ query.width_ratio_relative|unlocalize }}%" data-background-color="{{ query.trace_color }}">{{ query.width_ratio }}%</strong></div>
                </div>
            </td>
            <td class="djdt-time">
            ...
            <td class="djdt-actions">
                ...
                {% if query.params %} {% if query.is_select %}
                <form method="post">
                    {{ query.form }}

                    <button formaction="{% url 'djdt:sql_select' %}" class="remoteCall">Sel</button>
                    <button formaction="{% url 'djdt:sql_explain' %}" class="remoteCall">Expl</button> {% if query.vendor == 'mysql' %}
                    <button formaction="{% url 'djdt:sql_profile' %}" class="remoteCall">Prof</button> {% endif %}
                </form>
                {% endif %} {% endif %}
            </td>
        </tr>
        <tr class="djUnselected djDebugHoverable {% cycle 'djDebugOdd' 'djDebugEven' %}{% if query.is_slow %} djDebugRowWarning{% endif %} djToggleDetails_{{ forloop.counter }}" id="sqlDetails_{{ forloop.counter }}">
            ...
            <td colspan="4">
                <div class="djSQLDetailsDiv">
                    ...
                    <pre class="djdt-stack">{{ query.stacktrace }}</pre> {% endif %} {% if query.template_info %}
                    <table>
...
<script src="/staticfiles/debug_toolbar/js/toolbar.sql.js"></script>